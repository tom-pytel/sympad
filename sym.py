...
class ast2tex: # abstract syntax tree -> LaTeX text
	...
	_ast2tex_funcs = {
		...
		'-'    : lambda self, ast: f'-{self._ast2tex_wrap (ast.minus, ast.minus.is_num_pos or ast.minus.is_mul, {"=", "<>", "+", "slice", "||", "^^", "&&", "or", "and", "not"})}',
		'!'    : lambda self, ast: self._ast2tex_wrap (ast.fact, {'^'}, (ast.fact.op not in {'#', '@', '"', '(', '|', '!', '^', 'vec', 'mat'} or ast.fact.is_num_neg)) + '!',
		'+'    : _ast2tex_add,
		'*'    : _ast2tex_mul,
		...
		'sum'  : _ast2tex_sum,
		'diff' : _ast2tex_diff,
		'diffp': lambda self, ast: self._ast2tex_wrap (ast.diffp, 0, ast.diffp.is_pow) + "'" * ast.count,
		'intg' : _ast2tex_intg,
...
class ast2nat: # abstract syntax tree -> native text
	...
	_ast2nat_funcs = {
		...
		'!'    : lambda self, ast: self._ast2nat_wrap (ast.fact, {'^'}, ast.fact.op not in {'#', '@', '"', '(', '|', '!', '^', 'vec', 'mat'} or ast.fact.is_num_neg) + '!',
		'+'    : _ast2nat_add,
		'*'    : _ast2nat_mul,
		'/'    : _ast2nat_div,
		'^'    : _ast2nat_pow,
		...
		'sum'  : _ast2nat_sum,
		'diff' : _ast2nat_diff,
		'diffp': lambda self, ast: self._ast2nat_wrap (ast.diffp, 0, ast.diffp.is_pow) + "'" * ast.count,
		'intg' : _ast2nat_intg,
		'mat'  : _ast2nat_mat,
