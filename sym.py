...
class ast2nat: # abstract syntax tree -> native text
	...
	_ast2nat_funcs = {
		...
		'-ufunc': lambda self, ast: f'?{ast.ufunc}({", ".join (tuple (self._ast2nat (v) for v in ast.vars) + tuple (f"{k} = {self._ast2nat_wrap (a, 0, a.is_comma)}" for k, a in ast.kw))})',
...
class spt2ast:
	...
	def _spt2ast_AppliedUndef (self, spt):
		...
		return AST ('-ufunc', spt.name, tuple (self._spt2ast (a) for a in spt.args)) + \
			((tuple (sorted ((k, self._spt2ast (a)) for k, a in spt._extra_kwargs.items ())),) if spt._extra_kwargs else ()) # i._explicit_class_assumptions.items ()))
